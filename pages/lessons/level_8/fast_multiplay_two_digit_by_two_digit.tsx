...
import Description1 from '../../../components/descriptions/lessons/level_8/multTwoDigitByTwoDigit/Description1';
...
function FastMultiplayTwoDigitByTwoDigit() {
  ...
  const [example, set__example] =
    useState<generateExample__MultDifferentRangesForOperands>();
  ...
  const nextTask = () => {
    set__example(
      new generateExample__MultDifferentRangesForOperands(
        Number(min),
        Number(max),
        Number(min),
        Number(max)
  ...
  return (
    <>
      <Head>
        <title>Умножение двузначных чисел на двузначные</title>
        <meta
          name='description'
          content='Умножение двузначных чисел на двузначные '
        />
      </Head>
      <Grid container direction='column'>
        <Header
          hrefPrev='/lessons/level_7/myltiply_by_3'
          hrefNext='/'
          time={time}
...
export default FastMultiplayTwoDigitByTwoDigit;
